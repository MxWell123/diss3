package simulation;

import OSPABA.*;

public class Mc extends IdList
{
	//meta! userInfo="Generated code: do not modify", tag="begin"
	public static final int novyZakaznik = 1038;
	public static final int initPrichodyZakaznikov = 1002;
	public static final int koniec = 1042;
	public static final int prichodZakaznikaTerm1 = 1009;
	public static final int koniecObsluhy = 1048;
	public static final int prichodZakaznikaTerm2 = 1014;
	public static final int prichodZakaznikaNaVratenieAuta = 1015;
	public static final int initPrichodMinibusov = 1016;
	public static final int koniecNastupu = 1049;
	public static final int koniecVystupu = 1051;
	public static final int koniecPrechodu = 1052;
	public static final int nastupZakaznikovTerm1 = 1021;
	public static final int nastupZakaznikovTerm2 = 1024;
	public static final int novyMinibus = 1056;
	public static final int vystupZakaznikaTerm3 = 1061;
	public static final int vystupZakaznikaDoObsluhy = 1062;
	public static final int prichodZakaznikovNaCakanieNaMinibus = 1065;
	public static final int odchodZakaznikov = 1068;
	public static final int nastupZakaznikovZObsluhy = 1034;
	//meta! tag="end"

	// 1..1000 range reserved for user
}